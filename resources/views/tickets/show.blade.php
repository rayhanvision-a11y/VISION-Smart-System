<x-app-layout>
    ...
    const roleStyle = {
        ...
        noc:      { bg: '#dbeafe', text: '#1d4ed8' },
        reseller: { bg: '#d1fae5', text: '#065f46' },
    };
    ...
    // ── Build a chat bubble element from JSON ────────────────────────────────
    const myId = {{ auth()->id() }};

    function buildBubble(msg) {
        const isMe   = msg.isMe;
        const role   = msg.senderRole;
        const rs     = roleStyle[role] || { bg: '#f1f5f9', text: '#475569' };

        const wrap = document.createElement('div');
        wrap.classList.add('flex', isMe ? 'justify-end' : 'justify-start');
        wrap.dataset.messageId = msg.id;

        const avatarImg = `<img src="${msg.avatarUrl}" alt="${msg.senderName}" class="w-7 h-7 rounded-full object-cover flex-shrink-0 mt-1">`;
        let avatarLeft = '', avatarRight = '';
        if (!isMe) {
            avatarLeft = `<div class="mr-2">${avatarImg}</div>`;
        } else {
            avatarRight = `<div class="ml-2">${avatarImg}</div>`;
        }

        const roleTag = `<span style="background:${rs.bg};color:${rs.text}" class="inline-flex px-1.5 py-0.5 rounded text-xs font-medium">${role.toUpperCase()}</span>`;
        const meta    = isMe
            ? `<div class="flex items-center gap-2 mb-1 justify-end"><span class="text-xs font-semibold text-slate-600">${msg.senderName}</span>${roleTag}<span class="text-xs text-slate-400">${msg.time}</span></div>`
            : `<div class="flex items-center gap-2 mb-1"><span class="text-xs font-semibold text-slate-600">${msg.senderName}</span>${roleTag}<span class="text-xs text-slate-400">${msg.time}</span></div>`;

        let replyPreview = '';
        if (msg.replyTo) {
            replyPreview = `<div class="mb-1 px-2.5 py-1.5 rounded-lg bg-slate-50 border-l-2 border-indigo-300 text-xs text-slate-500 ${isMe ? 'text-right' : ''}"><span class="font-semibold text-slate-600">${escHtml(msg.replyTo.senderName)}</span>: ${escHtml(msg.replyTo.preview)}</div>`;
        }

        const bubbleBg = isMe ? 'background:#4f46e5;color:white' : 'background:#f1f5f9;color:#1e293b';
        let content = msg.message ? `<div class="text-sm leading-relaxed prose prose-sm max-w-none">${msg.message}</div>` : '';
        if (msg.image_url) {
            content += `<img src="${msg.image_url}" alt="Attachment" onclick="openImgModal(this.src)" style="max-height:160px;max-width:220px;object-fit:cover;display:block;border-radius:8px;margin-top:8px;cursor:pointer;transition:opacity .15s;" onmouseover="this.style.opacity='.85'" onmouseout="this.style.opacity='1'">`;
        }
        ...
        const quickEmojis = ['👍', '❤️', '😂', '😮', '😢', '🎉'];
        let quickBtns = quickEmojis.map(em => `<button type="button" class="msg-react-btn text-xs hover:scale-125 transition-transform p-0.5 rounded focus:outline-none" data-msg-id="${msg.id}" data-emoji="${em}" title="React ${em}">${em}</button>`).join('');

        const actions = `${reactionsHtml}<div class="flex items-center gap-2 mt-1.5 ${isMe ? 'justify-end' : ''}">
            <button type="button" class="msg-reply-btn text-xs text-slate-400 hover:text-indigo-600 font-medium transition-colors" data-id="${msg.id}" data-sender="${escHtml(msg.senderName)}" data-preview="${plainPreview}">Reply</button>
            <span class="text-slate-200 text-xs">|</span>
            <div class="inline-flex items-center gap-1 bg-slate-50 border border-slate-200/80 rounded-full px-2 py-0.5 shadow-2xs">${quickBtns}</div>
        </div>`;

        wrap.innerHTML = `${avatarLeft}<div class="max-w-sm">${meta}${replyPreview}<div style="${bubbleBg}" class="rounded-xl px-4 py-3 shadow-sm">${content}</div>${actions}</div>${avatarRight}`;

        // Alpine doesn't process dynamically-injected x-data automatically; init it.
        ...
        .then(data => {
            if (data.messages && data.messages.length > 0) {
                ...
                data.messages.forEach(msg => {
                    ...
                    if (commentsBox) commentsBox.insertBefore(bubble2, commentsBox.firstChild);
