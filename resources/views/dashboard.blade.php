<x-app-layout>
    ...
    <div class="mb-5 flex flex-wrap items-end justify-between gap-3">
        <div>
            <h1 class="text-2xl font-bold text-slate-800">{{ __($greeting) }}, {{ $user->name }}!</h1>
            <p class="text-slate-500 text-sm mt-1">{{ __("Here's what's happening with your tickets today.") }}</p>
        </div>

        @if($user->isAdmin() && isset($resellersQuick) && $resellersQuick->isNotEmpty())
        <div class="flex items-center gap-2">
            <label for="reseller_quick" class="text-xs font-semibold text-slate-500 uppercase tracking-wide">{{ __('Reseller') }}</label>
            <select id="reseller_quick"
                    onchange="if (this.value) window.location = this.value"
                    class="text-sm border-slate-300 rounded-lg focus:ring-indigo-500 focus:border-indigo-500">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-5 gap-3 mb-6">
        <a href="{{ route('tickets.index') }}" class="flex items-center gap-3 bg-white border border-slate-200 rounded-xl px-4 py-3 hover:shadow-sm transition-shadow group">
            <div class="w-9 h-9 bg-indigo-100 rounded-lg flex items-center justify-center flex-shrink-0"><svg class="w-4 h-4 text-indigo-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v10a2 2 0 002 2h8a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2"/></svg></div>
            <div><p id="stat-g-all" class="text-xl font-bold text-indigo-700">{{ $globalAll }}</p><p class="text-xs text-slate-400 group-hover:text-indigo-500 transition-colors">{{ __('All Tickets') }}</p></div>
        </a>
        <a href="{{ route('tickets.index', ['status' => 'in_progress']) }}" class="flex items-center gap-3 bg-white border border-slate-200 rounded-xl px-4 py-3 hover:shadow-sm transition-shadow group">
            <div class="w-9 h-9 bg-blue-100 rounded-lg flex items-center justify-center flex-shrink-0"><svg class="w-4 h-4 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/></svg></div>
            <div><p id="stat-g-inprog" class="text-xl font-bold text-blue-600">{{ $globalInProg }}</p><p class="text-xs text-slate-400 group-hover:text-blue-500 transition-colors">{{ __('In Progress') }}</p></div>
        </a>
        <a href="{{ route('tickets.index', ['status' => 'pending']) }}" class="flex items-center gap-3 bg-white border border-slate-200 rounded-xl px-4 py-3 hover:shadow-sm transition-shadow group">
            <div class="w-9 h-9 bg-orange-100 rounded-lg flex items-center justify-center flex-shrink-0"><svg class="w-4 h-4 text-orange-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/></svg></div>
            <div><p id="stat-g-pending" class="text-xl font-bold text-orange-500">{{ $globalPending }}</p><p class="text-xs text-slate-400 group-hover:text-orange-500 transition-colors">{{ __('Pending') }}</p></div>
        </a>
        <a href="{{ route('tickets.index', ['status' => 'waiting_for_customer_feedback']) }}" class="flex items-center gap-3 bg-white border border-slate-200 rounded-xl px-4 py-3 hover:shadow-sm transition-shadow group">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-7 gap-4 mb-8">

        <a href="{{ route('tickets.index', ['status' => 'in_progress']) }}"
           class="bg-white border border-slate-200 rounded-xl shadow-sm border-l-4 border-l-blue-400 p-4 hover:shadow-md transition-shadow group block">
            ...
            <p id="stat-in-progress" class="text-3xl font-bold text-blue-600">{{ $stats['in_progress'] }}</p>
        ...
        <a href="{{ route('tickets.index', ['status' => 'pending']) }}"
           class="bg-white border border-slate-200 rounded-xl shadow-sm border-l-4 border-l-orange-400 p-4 hover:shadow-md transition-shadow group block">
            <div class="flex items-center justify-between mb-2">
            ...
            <p id="stat-pending" class="text-3xl font-bold text-orange-500">{{ $stats['pending'] }}</p>
            <p class="text-xs text-slate-400 mt-1 group-hover:text-indigo-600 transition-colors">{{ __('View all') }} →</p>
        </a>

        <a href="{{ route('tickets.index', ['status' => 'waiting_for_customer_feedback']) }}"
           class="bg-white border border-slate-200 rounded-xl shadow-sm border-l-4 border-l-violet-400 p-4 hover:shadow-md transition-shadow group block">
            <div class="flex items-center justify-between mb-2">
            ...
            <p id="stat-waiting" class="text-3xl font-bold text-violet-600">{{ $stats['waiting_for_customer_feedback'] }}</p>
            <p class="text-xs text-slate-400 mt-1 group-hover:text-indigo-600 transition-colors">{{ __('View all') }} →</p>
        </a>
        ...
        <a href="{{ route('tickets.index', ['status' => 'resolved']) }}"
           class="bg-white border border-slate-200 rounded-xl shadow-sm border-l-4 border-l-emerald-400 p-4 hover:shadow-md transition-shadow group block">
            ...
            <p id="stat-resolved" class="text-3xl font-bold text-emerald-600">{{ $stats['resolved'] }}</p>
            <p class="text-xs text-slate-400 mt-1 group-hover:text-indigo-600 transition-colors">{{ __('View all') }} →</p>
        </a>

        <a href="{{ route('tickets.index') }}"
           class="bg-white border border-slate-200 rounded-xl shadow-sm border-l-4 border-l-indigo-400 p-4 hover:shadow-md transition-shadow group block">
            <div class="flex items-center justify-between mb-2">
        ...
        <a href="{{ route('tickets.index', ['status' => 'overdue']) }}"
           class="bg-white border border-slate-200 rounded-xl shadow-sm border-l-4 border-l-rose-600 p-4 hover:shadow-md transition-shadow group block">
            <div class="flex items-center justify-between mb-2">
            ...
            <p id="stat-overdue" class="text-3xl font-bold text-rose-700">{{ $stats['overdue'] }}</p>
            <p class="text-xs text-slate-400 mt-1 group-hover:text-indigo-600 transition-colors">{{ __('View all') }} →</p>
        </a>

        <div class="bg-white border border-slate-200 rounded-xl shadow-sm border-l-4 border-l-teal-500 p-4 block">
            <div class="flex items-center justify-between mb-2">
            ...
            <p id="stat-avg-resolution" class="text-3xl font-bold text-teal-600">{{ $stats['avg_resolution_time'] ?? 'N/A' }}</p>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-5 gap-4 mb-8">
        <a href="{{ route('tickets.index', ['assigned' => 'me', 'status' => 'in_progress']) }}" class="bg-white border border-slate-200 rounded-xl shadow-sm border-l-4 border-l-blue-400 p-4 hover:shadow-md transition-shadow group block">
        ...
        <a href="{{ route('tickets.index', ['assigned' => 'me', 'status' => 'pending']) }}" class="bg-white border border-slate-200 rounded-xl shadow-sm border-l-4 border-l-orange-400 p-4 hover:shadow-md transition-shadow group block">
        ...
        <a href="{{ route('tickets.index', ['assigned' => 'me', 'status' => 'waiting_for_customer_feedback']) }}" class="bg-white border border-slate-200 rounded-xl shadow-sm border-l-4 border-l-violet-400 p-4 hover:shadow-md transition-shadow group block">
            ...
            <p class="text-3xl font-bold text-violet-600">{{ $stats['waiting_for_customer_feedback'] }}</p>
            <p class="text-xs text-slate-400 mt-1 group-hover:text-violet-500 transition-colors">{{ __('View all') }} →</p>
        </a>
        <a href="{{ route('tickets.index', ['assigned' => 'me', 'status' => 'resolved']) }}" class="bg-white border border-slate-200 rounded-xl shadow-sm border-l-4 border-l-emerald-400 p-4 hover:shadow-md transition-shadow group block">
        ...
        <a href="{{ route('tickets.index', ['assigned' => 'me']) }}" class="bg-white border border-slate-200 rounded-xl shadow-sm border-l-4 border-l-indigo-400 p-4 hover:shadow-md transition-shadow group block">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
        <a href="{{ route('tickets.index', ['status' => 'in_progress']) }}" class="bg-white border border-slate-200 rounded-xl shadow-sm border-l-4 border-l-blue-400 p-4 hover:shadow-md transition-shadow group block">
            <div class="flex items-center justify-between mb-2"><span class="text-xs font-semibold text-slate-500 uppercase tracking-wide">{{ __('In Progress') }}</span><div class="w-8 h-8 bg-blue-50 rounded-lg flex items-center justify-center"><svg class="w-4 h-4 text-blue-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/></svg></div></div>
            <p class="text-3xl font-bold text-blue-600">{{ $stats['in_progress'] }}</p>
            <p class="text-xs text-slate-400 mt-1 group-hover:text-indigo-600 transition-colors">{{ __('View all') }} →</p>
        </a>
        <a href="{{ route('tickets.index', ['status' => 'pending']) }}" class="bg-white border border-slate-200 rounded-xl shadow-sm border-l-4 border-l-orange-400 p-4 hover:shadow-md transition-shadow group block">
            <div class="flex items-center justify-between mb-2"><span class="text-xs font-semibold text-slate-500 uppercase tracking-wide">{{ __('Pending') }}</span><div class="w-8 h-8 bg-orange-50 rounded-lg flex items-center justify-center"><svg class="w-4 h-4 text-orange-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/></svg></div></div>
            <p class="text-3xl font-bold text-orange-500">{{ $stats['pending'] }}</p>
            <p class="text-xs text-slate-400 mt-1 group-hover:text-indigo-600 transition-colors">{{ __('View all') }} →</p>
        </a>
        <a href="{{ route('tickets.index', ['status' => 'resolved']) }}" class="bg-white border border-slate-200 rounded-xl shadow-sm border-l-4 border-l-emerald-400 p-4 hover:shadow-md transition-shadow group block">
            <div class="flex items-center justify-between mb-2"><span class="text-xs font-semibold text-slate-500 uppercase tracking-wide">{{ __('Resolved') }}</span><div class="w-8 h-8 bg-emerald-50 rounded-lg flex items-center justify-center"><svg class="w-4 h-4 text-emerald-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg></div></div>
            <p class="text-3xl font-bold text-emerald-600">{{ $stats['resolved'] }}</p>
            <p class="text-xs text-slate-400 mt-1 group-hover:text-indigo-600 transition-colors">{{ __('View all') }} →</p>
        </a>
        <a href="{{ route('tickets.index') }}" class="bg-white border border-slate-200 rounded-xl shadow-sm border-l-4 border-l-indigo-400 p-4 hover:shadow-md transition-shadow group block">
